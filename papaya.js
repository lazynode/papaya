...
const getvaridx = (name, node, v = 0) => node === undefined ? (console.log(`[ERROR]: varname ${name} not found`) ?? 0) : node.$nodename$ !== `BasicStatementScope` ? getvaridx(name, node.$ancestor$, v) : node.varname === name ? v : getvaridx(name, node.$ancestor$, v + 1);
...
const BasicContractConstructor = ({ $ancestor$, $children$, split, tagsize }) => (uuid => ({ $nodename$: `AssemblySection`, $children$: [...$children$.slice(0, split), { $nodename$: `AssemblyPushTag`, tag: uuid, tagsize: tagsize }, { $nodename$: `AssemblyDUP1` }, { $nodename$: `AssemblyCODESIZE` }, { $nodename$: `AssemblySUB` }, { $nodename$: `AssemblyDUP1` }, { $nodename$: `AssemblySWAP2` }, { $nodename$: `AssemblyPUSH`, pushdata: '0x00' }, { $nodename$: `AssemblyCODECOPY` }, { $nodename$: `AssemblyPUSH`, pushdata: '0x00' }, { $nodename$: `AssemblyRETURN` }, { $nodename$: `AssemblyPlaceMark`, tag: uuid }, ...$children$.slice(split)] }))(crypto.randomUUID());
const BasicExpressionVarSwap = ({ $ancestor$, $children$, x, y }) => ((vx, vy) => vx > 15 || vy > 15 || vx == vy ? { $nodename$: `AssemblySection` } : { $nodename$: `BasicStatementStackSwap`, x: vx, y: vy })(getvaridx(x, $ancestor$), getvaridx(y, $ancestor$));
...
const AssemblyPlaceMark = ({ $ancestor$, $children$, tag }) => ({ $nodename$: `WriteData`, codedata: `0x`, tag: tag });
...
const WritePad = ({ $ancestor$, $children$, padsize }) => ({ $nodename$: `WriteData`, $children$: [...$children$, { $nodename$: `WriteData`, codedata: ethers.utils.hexZeroPad('0x', $children$.map(getleaves).flat().reduce((a, v) => a > ethers.utils.hexDataLength(v.codedata) ? a - ethers.utils.hexDataLength(v.codedata) : (console.log(`[ERROR]: padsize overflow`) ?? 0), padsize)) }] });
